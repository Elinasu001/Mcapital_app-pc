...
$.fn.pageMaker = function(options){
	...
	if( o.pageDefault ){
		...
		if( o.pageDefault.pageType == null ){
			...
			page.each(function(){
				...
				// [no_calc]
				// if( _this.hasClass('no_calc') == true ){
				// 	var pageDate = _this.find('.info.date')
				// };
				...
				//---------- 20220209 remix added
				if( _this.hasClass('both') == true ){
					_this.find('.info.title').append('<div class="type-ch">CH</div>')
					_this.find('.info.title').append('<div class="type-cu">CU</div>')
				}

				if ( _this.hasClass('ch') == true ){
					_this.find('.row_filename').append('<div class="type-ch">CH</div>')
				}

				if ( _this.hasClass('cu') == true ){
					_this.find('.row_filename').append('<div class="type-cu">CU</div>')
				}
				//---------- 20220209 remix added //
	...
	if( o.manageMode ){
		...
		$('.version_wrap .ver').text(version);
		$('.version_wrap .date').text(recentDateTxt);
		$('.pub-notice h1 span').text(recentDateTxt);
		...
		pageDate.each(function(){
			...
			if ( releaseDate == 'TBD' || releaseDate == '일부TBD' || releaseDate == '스토리보드X' ) {
				_this.closest('.page').addClass('tbd')
				if ( o.addTbd ){
...
var iaMenuCtrl = function(){
	...
	cloneArea.each(function(){
		...
		var _cloneP = $('.clone-target');
		var _cloneTarget =  _cloneP.find('[data-clonename="'+_cloneTargetName+'"]')
